    ...
    <div class="page-header float-right">
      <div class="page-title">
        <ol class="breadcrumb text-right">
          <li class="active"> @isset($patient) Edit Staff @else Add New Staff @endisset</li>
        </ol>
      </div>
    </div>
  </div>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <strong> @isset($patient) Edit {{$patient->name}} Staff @else Add New Patient @endisset</strong>
        </div>
        <div class="card-body card-block">
              <form method="post" action="@isset($patient) {{ route('staff-edit-patient') }}  @else {{ route('staff-add-patient') }} @endisset">
                @csrf
                @isset($patient->username) <input type="hidden" name="id" value="{{ $patient->id }}"> @endisset
